...
SELECT purchase_date_time, type 
FROM tmb.card WHERE (uses_left > 0 or uses_left IS NULL) 
AND user_ID = "saach3" AND
  CURDATE() <= expiration_date


SELECT user.first_name, user.last_name, review.station_name,
review.shopping, review.connection_speed, review.comment
FROM user JOIN review 
ON user.ID=review.passenger_ID
WHERE review.approval_status="pending";